(function($) {
  ...
  $.backgroundVideo = function(el, options) {
    ...
    var buildVideo = function () {
      ...
      block += '<video id="'+plugin.settings.videoid+'"' + preloadString + autoplayString + loopString;

      if (plugin.settings.poster) {
        block += ' poster="' + plugin.settings.poster + '" ';
      }

     block += 'style="position:absolute;top:0;left:0;bottom:0;right:0;z-index:-100;width:100%;height:100vh;">';
      for(var i=0; i < plugin.settings.types.length; i++) {
        block += '<source src="'+plugin.settings.path+plugin.settings.filename+'.'+plugin.settings.types[i]+'" type="video/'+plugin.settings.types[i]+'" />';
      }
      block += 'bgvideo</video>';
      plugin.el.html(block);
